...
- **로컬 전체 검증(.venv): 2624 passed, 13 skipped, 1 xfailed, 0 failed.**
- **NER 품질**: `thresholds_met=true` — ADDRESS P=1.00/R=0.88, ORGANIZATION P=1.00/R=0.76
  (regex 대비 recall +0.76), PERSON 목표 충족. (`benchmarks/korean_ner_benchmark.py`)
...
.venv/bin/python -m pytest -q -p no:cacheprovider      # 2624 passed
# .venv 없으면: pip install -e ".[ner,dev]" && python -m spacy download ko_core_news_sm
