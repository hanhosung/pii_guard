...
### 2.3 Stage 2 모델 비교 — spaCy vs GLiNER vs NuNER Zero

Stage2 NER은 **선택형 백엔드**다: 기본 **`gliner`**(모델 `urchade/gliner_multi_pii-v1`, Apache-2.0·상업 가능), 경량 폴백 **`spacy`**(Presidio + `ko_core_news_lg`), 평가 후보 **`nunerzero`**(`numind/NuNER_Zero`, MIT·상업 가능, R21·ADR-14 — 아래 §2.5). 선택 = `PIIGUARD_NER_BACKEND` env 또는 정책 `stage2.ner_backend`(env 우선). 세 백엔드는 동일 카테고리(PERSON/ADDRESS/ORGANIZATION)로 정규화되어 후단(정책·마스킹·proximity 후필터·degrade)은 백엔드와 무관.

**① 라벨 코퍼스(합성·ground-truth, 순수 모델 품질, spf=5·임계값 0.50)**

| 카테고리 | spaCy P / R | GLiNER P / R | NuNER Zero P / R |
| :-- | :-- | :-- | :-- |
| PERSON | 0.974 / 0.844 | 0.935 / **0.956** | 0.974 / 0.844 |
| ADDRESS | 1.000 / 1.000 | 0.917 / 0.880 | 0.962 / **1.000** |
| ORGANIZATION | 1.000 / 0.920 | 0.774 / **0.960** | 0.809 / 0.680 |

**② 외부 LLM 생성 6개 리포트(현실형 한·영 혼합, full-pipeline, Stage1 보강 후)**

| 데이터셋 | spaCy R / P | GLiNER R / P | NuNER Zero R / P |
| :-- | :-- | :-- | :-- |
| claude (30) | 0.941 / 0.877 | **0.966** / 0.872 | 0.863 / 0.884 |
| codex (10) | 0.921 / 0.828 | **0.989 / 0.936** | 0.899 / 0.941 |
| gemini (10) | 0.958 / 0.651 | **0.931 / 0.918** | 0.819 / 0.908 |

> NuNER Zero 외부 수치는 원본 입력 JSON이 없어 GLiNER 리포트에서 50케이스(텍스트·정답)를 복원해 동일 하니스(`validation/external_replay.py`)로 재채점한 값이며, **GLiNER 재채점치가 위 표와 정확히 일치해 하니스가 검증**됐다.

**해석**
- **재현율**: GLiNER가 코퍼스(PERSON 0.956·ORG 0.960)와 외부(claude·codex)에서 우위, gemini만 소폭 열세. 대체로 동등~우위. **NuNER Zero는 PERSON·ORG 재현율이 낮고**(코퍼스 ORG 0.680, 외부 PERSON 0.20까지 붕괴) 외부 종합 재현율이 세 백엔드 중 최저.
- **정밀도**: 코퍼스에선 spaCy가 높지만(특히 ORG 1.0 vs 0.774), **현실형 데이터에선 GLiNER가 크게 우위**(codex 0.94·gemini 0.92 vs spaCy 0.83·0.65). spaCy는 영문 로그 토큰(`auth`·`webhook`)을 인물/조직으로 과잉 추출 → gemini 오탐 37 vs GLiNER 6. NuNER Zero는 정밀도(FP↓)·ADDRESS에서만 강점.
- **결론**: 보안(유출 방지=recall)과 운영 품질(과잉 마스킹↓=precision) 모두에서 **기본 GLiNER(Apache)가 합리적**, 저메모리 환경은 `spacy` 폴백. NuNER Zero는 재현율 열세로 부적합(§2.5). 약점은 **GLiNER의 ORG 정밀도(0.774)** 한 곳에 집중.
- 라이선스: 기본 GLiNER `urchade/...`=Apache-2.0(상업 가능). 한국어 특화 `taeminlee/gliner_ko`는 성능 동등이나 **CC-BY-NC(비상업)**라 기본값 제외. spaCy `ko_core_news_lg`=CC BY-SA(런타임 다운로드만 하면 상업 OK). NuNER Zero `numind/NuNER_Zero`=MIT(상업 가능).
...
### 2.5 NuNER Zero 백엔드 후보 평가 (R21 · ADR-14)

GLiNER와 유사 성능을 내는 동계열 제로샷 인코더 NER **NuNER Zero**(`numind/NuNER_Zero`, **MIT·상업 가능**)를 GLiNER 대안 후보로 평가했다. GLiNER 라이브러리로 로드되는 **토큰 분류** 모델이라, 인접 조각을 잇는 병합 후처리(`_merge_adjacent_entities`)를 더해 동일 추상화(`resolve_ner_backend`)에 세 번째 백엔드(`nunerzero`)로 배선했다.

**평가 절차(배선 → 벤치 → 게이트)**: 코퍼스(seed=42) + 외부 6리포트 복원셋을 GLiNER와 동일 하니스로 재채점하고, 채택 게이트 — (a) 어떤 카테고리도 recall 회귀 없음 (b) ORG 정밀도 개선 또는 macro-F1 우위 (c) 런타임 예산 충족 — 으로 판정.

**결과 = 게이트 FAIL ❌ → GLiNER 기본 유지.**
- **강점(가설 일부 입증)**: ADDRESS 우위(병합 후처리로 코퍼스 R 1.00)·정밀도 전반(FP↓)·ORG 정밀도 개선(0.774→0.81).
- **결격(채택 차단)**: **PERSON·ORG 재현율 회귀**(코퍼스 ORG 0.96→0.68), 특히 **현실형 외부 데이터에서 PERSON 재현율 붕괴**(codex/gemini 1.00/0.80 → 0.20). 보안=재현율 1순위 도구에서 치명적이라 §2.3 표대로 GLiNER가 종합 우위.
- 어댑터·벤치·하니스는 모두 상주(`stage2/nunerzero_ner.py`·`benchmarks/compare_ner_backends.py`·`validation/external_replay.py`)해 ORG hard-negative 관점 등 향후 재평가에 즉시 재사용 가능. 상세 = `validation/NUNERZERO_TEST_2026-06-29.md` · `validation/NER_BACKEND_COMPARISON.md` §6.
...
## 부록 — 검증 재현
- 코퍼스 벤치마크: `benchmarks/korean_ner_benchmark.py --ner-backend {spacy,gliner,nunerzero}`
- 외부 리포트: `validation/EXTERNAL_LLM_TEST_2026-06-23_{claude,codex,gemini}_{spaCy,GLiNER}.md`
- 종합 비교/임계값 스윕(§6 NuNER Zero 포함): `validation/NER_BACKEND_COMPARISON.md`
- NuNER Zero 후보 평가(R21·ADR-14): `validation/NUNERZERO_TEST_2026-06-29.md` · 백엔드 대조+게이트 `benchmarks/compare_ner_backends.py` · 외부 재생성 `validation/external_replay.py`
