...
### 3.1 탐지 엔진 역할 분담 — NLP 인코더 NER(탐지기) + 로컬 EXAONE(데이터 엔진) (DR-1)
...
#### (3) 결론
보안 탐지기에 생성형 LLM(외부든 로컬 EXAONE든)을 인라인 1차로 두는 것은 퇴보다. "더 똑똑한 탐지"의 정답은 **인코더 NER 모델 파인튜닝/교체**(§2.4, §3.2)이고, **로컬 EXAONE는 그 파인튜닝의 데이터·평가 엔진**으로 기여한다. 규칙(정규식·체크섬)은 고심각·정형 카테고리에서 계속 1급으로 유지한다.
