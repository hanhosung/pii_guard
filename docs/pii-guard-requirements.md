...
- **엔진**: Microsoft Presidio + **spaCy 한국어 모델**. as-built 기본은 **`ko_core_news_lg`**(경량 폴백 `sm`), 8GB RAM 고려. 정확도가 더 필요하면 transformer NER(KoELECTRA/KLUE)로 **교체 가능하게 추상화**(`resolve_ko_spacy_model`). 자세한 선택 근거는 [`DESIGN.md`](./DESIGN.md) ADR-9·ADR-10.
- **하는 일**: 정규식이 못 잡는 **문맥상 이름·주소·조직**을 의미로 탐지.
- **블록당 하드 타임아웃**(기본 ~500ms~1s): 모델이 멈추거나 너무 느리면 무한 대기하지 않고 **끊고 `stage2_fail_action`으로 처리**(§14 열화). → 가용성 보장.
- **격리**: 무거운 모델은 **별도 워커 프로세스**(spawn)에서 돈다 → Stage2가 죽어도(OOM 등) 코어는 생존(§3.1).
- **구현**: `stage2/korean_ner.py`(엔진) + `stage2/runner.py`(워커·타임아웃·열화).
