...
| :-- | :-- | :-- | :-- |
| **GLiNER** (`gliner`, 한국어 특화 모델) | 제로샷 NER 엔진(**Stage2 기본 백엔드**) | 정규식이 못 잡는 **문맥상 이름·주소·조직**을 라벨 프롬프트(`사람`·`주소`·`조직`)로 직접 추출 → PERSON/ADDRESS/ORGANIZATION 매핑. 트랜스포머 기반(더 높은 재현율)이라 별도 워커·옵션 설치(`[ner-gliner]`)로 격리 | `stage2/gliner_ner.py` |
...
- **선택형 백엔드 (R18)**: Stage2 NER 엔진은 **두 가지 중 하나를 선택**해 동작한다. 두 백엔드 모두 같은 입력→같은 PII-Guard 카테고리(PERSON/ADDRESS/ORGANIZATION)로 정규화되며, 후단(정책·마스킹·proximity 후필터)은 백엔드와 무관하게 동일하다.
  ...
  | 백엔드 | 엔진 | 위치 | 성격 |
  | :-- | :-- | :-- | :-- |
  | **`gliner` (기본)** | GLiNER **한국어 특화 모델**(예: `taeminlee/gliner_ko`). 라벨 프롬프트(`사람`·`주소`·`조직`)로 제로샷 추출 후 카테고리 매핑 | `stage2/gliner_ner.py` | 트랜스포머 기반 → **재현율 우선**. PyTorch 의존(더 무거움). 옵션 설치 `[ner-gliner]` |
...
- GLiNER 백엔드 배선·실측(R18, 진행 대상) → 추가로 transformer 한국어 NER(KoELECTRA/KLUE) 옵션, 서명된 자동 룰 갱신 채널
...
| 미정 항목 | 확정 내용 |
| :-- | :-- |
| **NER 백엔드/모델** | **선택형 백엔드 채택·배선 완료(R18)** — 기본 **`gliner`**(한국어 특화 GLiNER, 재현율 우선), 경량 폴백 **`spacy`**(`ko_core_news_lg`, sm 재폴백). 백엔드 선택 = `PIIGUARD_NER_BACKEND` env 또는 정책 `stage2.ner_backend`(env 우선). 모델 변형 = GLiNER `PIIGUARD_GLINER_MODEL` / spaCy `PIIGUARD_KO_SPACY_MODEL`. **spaCy(lg) 실측**: full-pipeline recall **PERSON 0.84, ADDRESS 1.00, ORGANIZATION 0.92**(정밀도 유지). **GLiNER 실측**(동일 코퍼스, `taeminlee/gliner_ko`): recall **PERSON 0.978, ADDRESS 1.00, ORGANIZATION 1.00**, precision **PERSON 0.80, ADDRESS 1.00, ORGANIZATION 0.86** — 재현율↑·정밀도↓(전 임계값 통과, `validation/gliner_benchmark.json`). 상세 = DESIGN §6.4·ADR-11. |
...
| 한계 | 내용 | 영향 / 완화 |
| :-- | :-- | :-- |
| ★ **KR_ACCOUNT 비표준 포맷 재현율 갭** | 계좌번호 탐지는 **인식된 은행 구획**(6-2-6/3-6-5/4-3-6) 또는 **"계좌번호:" 라벨**이 있을 때만 동작. 라벨 없는 `xxx-xxx-xxxxxx`(3-3-6)·토스/카카오 3-2-7 등은 **오탐 억제를 위해 일부러 미탐지**(검증 recall 0.78). | 은연중 흘린 비표준 계좌 통과 가능. **완화 = R17 양성 proximity**(은행명+입금/계좌 근접 시 승격). |
| ★ **PASSWORD 한글 라벨 미검출** | "비밀번호: …" 같은 **한글 라벨**은 미탐지(영문 `password=` 키워드만 인식, 검증 recall 0.80). | **완화 = R17 양성 proximity**(한글 "비밀번호/비번/암호" 트리거 추가). |
...
| **PERSON recall < 1.0** | spaCy lg 기준 0.84(작은 모델 한계). honorific 없는 맨이름은 일부 누락. | 고심각 아님(이름=mask). 완화 = **R18 GLiNER 백엔드로 전환**(기본값, 목표 recall ≥0.90) / honorific 룰 / KoELECTRA·KLUE 교체. |
