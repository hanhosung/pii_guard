...
- ❌ 순수 "신뢰-부주의" 모델 배제: 코딩 에이전트는 비신뢰 콘텐츠를 읽고 조종될 수 있어, 적대적 우회를 범위 밖으로 두면 현대 1급 위협에 무력.
- ❌ 순수 "비신뢰-봉쇄 컨테이너" 모델 배제: 같은 머신·같은 파일시스템에서 base_url 되돌리기·프로세스 종료가 가능해 **봉쇄를 표방하는 것 자체가 거짓 안심**. 진짜 봉쇄는 VM/샌드박스라는 다른 제품.
- ✅ 채택: **데이터플레인은 섬기고, 컨트롤플레인은 격리**한다.

### 2.2 신뢰 경계 (control plane / data plane 분리) — 비협상 요구사항

| 플레인 | 구성 | 에이전트 권한 |
| :-- | :-- | :-- |
| **컨트롤 플레인** | 정책 파일, allowlist/pin-list, **HMAC 키**, Ledger, egress 락다운 규칙, 프록시 프로세스 | **쓰기 불가** (별도 UID 또는 root 소유) |
| **데이터 플레인** | 에이전트 프로젝트 파일·작업 디렉토리 | 읽기/쓰기 |

- pin-list/allowlist 변경 = 에이전트가 아닌 **사용자의 out-of-band 승인**. (에이전트가 자기 유출을 화이트리스트 못 함.)
- egress 락다운 pf 규칙 = **root 소유**, 비root 에이전트가 제거 불가.
- 프록시 = 에이전트가 권한 없이 못 죽이는 별도 프로세스/유저.
