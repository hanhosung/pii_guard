...
> **구현 상태(정직 선언, P3)**: dual-backend **설계·배선 모두 구현 완료**. `stage2/backend.py`(`resolve_ner_backend`)·`stage2/gliner_ner.py`(`GLiNERNEREngine`)·`_workers.py` 분기·`policy.py` `stage2.ner_backend` 파싱·`Engine(ner_backend=…)` env 전파·`serve` 연결이 모두 배선되고 **선택 로직은 단위 테스트로 검증**됨(`tests/test_ner_backend.py`). 다만 **GLiNER 런타임 동작·품질 수치는 실측 미완**이다 — 본 개발 환경에 `gliner`/`torch` 미설치(spaCy만 설치)라 GLiNER 추론은 `pip install 'pii-guard[ner-gliner]'` 후에야 동작/측정 가능. 따라서 GLiNER 품질 수치(§23.1·DESIGN §6.4)는 **측정 예정(목표)**으로 두며, 실측 전까지 보증선은 spaCy 실측치다.
...
| 미정 항목 | 확정 내용 |
| :-- | :-- |
| **NER 백엔드/모델** | **선택형 백엔드 채택·배선 완료(R18)** — 기본 **`gliner`**(한국어 특화 GLiNER, 재현율 우선), 경량 폴백 **`spacy`**(`ko_core_news_lg`, sm 재폴백). 백엔드 선택 = `PIIGUARD_NER_BACKEND` env 또는 정책 `stage2.ner_backend`(env 우선). 모델 변형 = GLiNER `PIIGUARD_GLINER_MODEL` / spaCy `PIIGUARD_KO_SPACY_MODEL`. **spaCy(lg) 실측 보증선**: 한국어 코퍼스 full-pipeline recall **PERSON 0.84, ADDRESS 1.00, ORGANIZATION 0.92**(정밀도 유지, `thresholds_met=true`). **GLiNER 실측치는 측정 예정**(목표: PERSON recall ≥ 0.90, `[ner-gliner]` 설치 후). 상세 = DESIGN ADR-11. |
