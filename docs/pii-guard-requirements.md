...
> **구현 상태(정직 선언, P3)**: 위 dual-backend **설계는 확정**이며 spaCy 백엔드는 현재 배선·검증되어 있다. **GLiNER 백엔드(`stage2/gliner_ner.py`)와 `resolve_ner_backend()` 배선은 본 개정으로 확정된 신규 요구사항 R18로, 코드 구현이 진행 대상**이다. GLiNER 품질 수치(§23.1·DESIGN §6.4)는 **측정 예정(목표)**으로 표기하며, 실측 전까지는 spaCy 실측치가 보증선이다.
...
| 미정 항목 | 확정 내용 |
| :-- | :-- |
| **NER 백엔드/모델** | **선택형 백엔드 채택(R18)** — 기본 **`gliner`**(한국어 특화 GLiNER, 재현율 우선), 경량 폴백 **`spacy`**(`ko_core_news_lg`, sm 재폴백). 백엔드 선택 = `PIIGUARD_NER_BACKEND` env 또는 정책 `stage2.ner_backend`(env 우선). 모델 변형 = GLiNER `PIIGUARD_GLINER_MODEL` / spaCy `PIIGUARD_KO_SPACY_MODEL`. **spaCy(lg) 실측 보증선**: 한국어 코퍼스 full-pipeline recall **PERSON 0.84, ADDRESS 1.00, ORGANIZATION 0.92**(정밀도 유지, `thresholds_met=true`). **GLiNER 실측치는 측정 예정**(목표: PERSON recall ≥ 0.90). 상세 = DESIGN ADR-11. |
