...
[이 파일이 하는 일 — 한 줄 요약]
Stage2 NER 엔진을 **GLiNER(기본)와 spaCy(경량 폴백)** 중 무엇으로 돌릴지 결정한다.
선택 우선순위는 **환경변수 `PIIGUARD_NER_BACKEND` > 정책 `stage2.ner_backend` > 기본 `gliner`**.
두 엔진은 동일한 `.detect(text) -> List[Detection]` 인터페이스를 가지므로, 워커 루프는
여기서 고른 엔진 클래스를 지연 임포트해 쓰기만 하면 백엔드와 무관하게 동작한다.
...
class NERBackend(str, Enum):
    ...
    GLINER = "gliner"   # 한국어 특화 GLiNER(기본)
...
def load_engine_class(backend: NERBackend):
    ...
    무거운 의존(GLiNER+PyTorch, spaCy+Presidio)이 부모(코어) 프로세스에 올라가지 않도록,
    임포트를 이 함수 호출 시점(=워커 서브프로세스 안)으로 미룬다.
    반환된 클래스는 둘 다 `__init__()` 후 `.detect(text) -> List[Detection]`를 제공한다.
    """
