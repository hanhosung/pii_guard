...
정형 PII(API키·카드·여권·이메일·전화)는 사실상 1.00. 전체 **2675 테스트 통과**. 상세 — [`validation/EFFICACY_REPORT.md`](validation/EFFICACY_REPORT.md), 개선 전후 — [`validation/EFFICACY_BEFORE_AFTER.md`](validation/EFFICACY_BEFORE_AFTER.md).
...
```bash
.venv/bin/python -m pytest -q          # 2675 passed (NER 통합테스트 포함)
```
...
- [실효성 검증](validation/EFFICACY_REPORT.md) · [Before/After](validation/EFFICACY_BEFORE_AFTER.md)
