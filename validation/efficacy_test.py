...
def generate_md(cases, per_cat, summary):
    ...
    W("## 2. 핵심 결과")
    W()
    W("| 지표 | Raw | Triaged(보정) | 설명 |")
    W("| :-- | :-- | :-- | :-- |")
    W(f"| **재현율(Recall)** | **{rec_raw:.3f}** ({tp}/{tp+fn}) | **{rec_adj:.3f}** ({tp}/{tp+real_fn}) | "
      f"보정=출제오류 FN {auth}건 제외 |")
    W(f"| **정밀도(Precision)** | **{prec_raw:.3f}** ({tp}/{tp+fp}) | **{prec_adj:.3f}** ({tp_adj}/{tp_adj+true_fp}) | "
      f"보정=비라벨 정탐 {legit}건 반영 |")
    W()
    W(f"- ground truth 총 **{summary['total_exp']}개** PII 항목 · 검출 **{tp}** · 미검출 **{fn}** · "
      f"오탐 후보 **{fp}**(이 중 진짜 over-masking **{true_fp}**).")
    W(f"- block 카테고리(시크릿·주민번호 등) 포함 케이스는 프록시에서 **요청 차단** 대상.")
    W()
    W("## 3. 카테고리별 재현율")
    W()
    W("| 카테고리 | TP | FN | recall |")
