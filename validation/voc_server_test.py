...
def generate_md(s, items):
    ...
    si_mark = "✅" if s["info_r"] >= 0.85 else "⚠️"
    W(f"| {si_mark} **서버정보**(내부 IP·호스트명) | 30 | {s['info']['tp']+s['info']['fn']} | {s['info']['tp']} | **{s['info_r']:.3f}** |")
    W("")
    if s["info_r"] >= 0.85:
        W("> **요약**: 고객 PII·서버 시크릿·서버정보(IP/호스트명) **세 가지 모두 높은 비율로 마스킹/차단**됨. "
          "내부 IP·호스트명은 `IP_ADDRESS`/`HOSTNAME` 카테고리로 마스킹되어 외부 LLM에 네트워크 토폴로지가 노출되지 않음.")
    else:
        W(f"> **요약**: 고객 PII와 서버 시크릿은 높은 비율로 차단/마스킹되지만, **내부 IP·호스트명은 "
          f"전용 카테고리가 없어 검출률 {s['info_r']:.2f}**(범위 밖) — IP/HOSTNAME 카테고리 추가 권고.")
    W("")
    W("## 2. 카테고리별")
    W("")
    W("| 카테고리 | TP | FN | recall |")
    ...
    W("- ✅ **서버 시크릿**: 로그 속 API키·AWS키·JWT·프라이빗키·DB비밀번호가 차단(block) → "
      "**자격증명 유출 차단**.")
    si_line = ("- ✅ **서버정보(IP·호스트명)**: `IP_ADDRESS`/`HOSTNAME` 카테고리로 마스킹 → "
               "**내부 네트워크 토폴로지가 외부 LLM에 노출되지 않음**. 공개 도메인(api.anthropic.com 등)은 "
               "보존하여 정상 참조는 유지.") if s["info_r"] >= 0.85 else \
              ("- ⚠️ **서버정보(IP·호스트명)**: 전용 카테고리 부재로 대부분 통과 → IP/HOSTNAME 카테고리 추가 권고.")
    W(si_line)
